...
def upload_to_onedrive(excel_file: str, timeout_seconds: int = 300):  # Default 5 minutes timeout
    """
    Upload an Excel file to OneDrive with a timeout.
    
    Args:
        excel_file: Path to the Excel file to upload
        timeout_seconds: Maximum time in seconds to allow for the upload process
    
    Raises:
        TimeoutError: If the upload process exceeds the timeout
    ...
    if "access_token" in result:
        ...
        try:
            ...
            if response.status_code in (200, 201):
                send_telegram_message("File successfully uploaded to OneDrive.")
                print("File successfully uploaded to OneDrive.")
            else:
                send_telegram_message(f"Upload failed: {response.status_code} - {response.json()}")
                print(f"Upload failed: {response.status_code} - {response.json()}")
        except Exception as e:
            send_telegram_message(f"Exception during file upload: {e}")
            print(f"Exception during file upload: {e}")
    else:
        send_telegram_message(f"Failed to obtain access token: {result.get("error_description")}")
